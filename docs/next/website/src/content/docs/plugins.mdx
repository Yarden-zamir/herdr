...
A Ctrl-click on a visible file path also reaches handlers, as a `file://` URL,
when the path exists relative to the pane working directory. Absolute, `~/`,
`./`, and plain relative paths qualify; a trailing `:line[:col]` becomes a
`#L<line>` fragment. Herdr never opens `file://` URLs itself: without a
matching handler the click stays a plain click.
